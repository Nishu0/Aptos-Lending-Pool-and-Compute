module nisarg::nisarg {
    ...
    const COLLECTION_NAME: vector<u8> = b"Nisarg Collection";
    const COLLECTION_DESCRIPTION: vector<u8> = b"Nisarg Whitelist NFT";
    ...
    fun get_token_signer(): signer acquires CollectionCapability {
        account::create_signer_with_capability(&borrow_global<CollectionCapability>(@nisarg).capability)
    }
    ...
    public entry fun create_token(
        ...
        let token_name = string::utf8(b"Nisarg Token");
        let mint_info = borrow_global_mut<MintInfo>(@nisarg);
